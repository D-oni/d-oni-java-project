...
### 훈련 1: 동물 데이터를 파일에 보관하라.

- App.java
    - 애플리케이션을 실행했을 때 파일에서 동물 데이터를 읽어오는 `loadAnimalData()`를 정의한다.
    - 애플리케이션을 종료할 때 동물 데이터를 파일에 저장하는 `saveAnimalData()`를 정의한다.
    - 동물 데이터를 저장할 ArrayList 객체는 위에서 만든 메서드에서 접근할 수 있도록 스태틱 필드로 전환한다.

#### 실행 결과

`App`을 실행한 후 동 데이터를 입력한다.
```
명령> /animal/add
번호: 1
이름: 나비
설명: 머리 위 작은 반점
1.유기동물 입양  2.유기동물 보호  3.반려동물 찾기 : 3
동물등록번호여부(Y/n): Y
...
```
명령> /animal/list
  1, 나  비    , 2019-01-01 ~ 2019-05-05,  160
```

### 훈련 2: 회원 데이터를 파일에 보관하라.

- App.java
    - 애플리케이션을 실행했을 때 파일에서 회원 데이터를 읽어오는 `loadInfoData()`를 정의한다.
    - 애플리케이션을 종료할 때 회원 데이터를 파일에 저장하는 `saveInfoData()`를 정의한다.
